...
from trainers.SegnetTransTrainer import SegnetTrainer
...
def main():
    ...
    # prepare dataloader
    dataset = TransferDataloader(cfg.DATASETS.name, split_set=('train', 'val',), max_iters=cfg.SOLVER.max_iters)
    models = build_model()
    ...
    mytrainer = SegnetTrainer(models, optimizers, lr_decays, dataset, sets=cfg.DATASETS.split, gpu_id=cfg.MODEL.gpu_id,
                              use_load_checkpoint=-1, net_type="deeplabv3")
